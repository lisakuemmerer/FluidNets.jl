...
# works not on all files
function read_data(path, var_dim)

    names = readdlm((path))[1,:]
    filter!(s->s!="[GeV]", names)
    var_labels = names[1:var_dim]
    K_labels = names[var_dim+1:end]

    data = readdlm((path), Float64, skipstart=1)

    # variables, Kernels in matrix form
    var_set = transpose(data[:,1:var_dim])
    K_set = transpose(data[:,var_dim+1:end])

    return var_labels, var_set, K_labels, K_set  # can get whole set with 'vcat(var_set, K_set)'
end
...
# in order, first var rises first
...
# auxfuc to get means&std of a set: (means-array, std-array)
...
# auxfuc to get minimum&widthof a set: (mins-array, widths-array)
...
# auxfuc to get zeros & absolute maximum of a set: (zeros-array, absmax-array)
...
function get_var_set(var_range; n=100, rng=def_rng)
...
# returns random variable set in matrix form (#vars, #samples)
function get_sorted_var_set(var_range; n=10, rng=def_rng)
...
function get_val_set(var_range, K_func, trainstate::model_structures; n=100)
    var_val_set = get_sorted_var_set(var_range, n=n)
...
function get_train_test_set(var_range::Vector, K_func::Vector; sort=false, preprocess_vars=false, var_prep_range=nothing, preprocess_K=false, K_prep_range=nothing, K_scale=1, n_train=10000, n_test=10000)
    var_dim = size(var_range)[1]

    K_dim = size(K_func)[1]

    sort==true ? var_train_set = get_sorted_var_set(var_range, n=n_train) : var_train_set = get_var_set(var_range, n=n_train)
    K_train_set = Kernels(var_train_set, K_func)

    sort==true ? var_test_set = get_sorted_var_set(var_range, n=n_test) : var_test_set = get_var_set(var_range, n=n_test)
...
# save & load trialarray with trainstates, losses (bigger, but not ordered dictionaries)
function deepsave_trials(trial_array, filename)
    open(filename, "w") do f
        write(f, JSON.json(trial_array))
    end
end
function deepload_trials(filename)
    string_d = JSON.parsefile(filename)
    symbol_d = []
    for d in string_d
